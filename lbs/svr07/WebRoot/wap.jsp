...
<title>地图</title>
<style type="text/css">
body,html,#allmap {width: 99%;height: 98%;overflow: hidden;margin:0;}
...
var i=1;
var station_lat=0;station_lng=0;
point_count=0;
while(i<fld.length)
{
	st=parseInt(fld[i++]);
	...
	else
	{
		...
		if((lat==station_lat) &&(lon=station_lng))
			{st=0;}
		else
			{
			station_lat=lat;
...
function callback(xyResults){
 ...
 var j=0;
 if(point_count==0) {map.clearOverlays();}
 ...
 // if (index==(xyResults.length -1))	marker.setLabel(label_time); //添加GPS标注
	  ...
	  {
	  var circle = new BMap.Circle(pointA,330,{strokeColor:"blue", strokeWeight:2, strokeOpacity:0.2,fillOpacity:0.04,strokeStyle:"dashed",enableMassClear:false});
		map.addOverlay(circle);
	  var marker = new BMap.Marker(pointA,{icon:myIcon});
     	map.addOverlay(marker);
...
function latest(datas)
{
//	alert(datas);
//	map.clearOverlays();
	showmap(datas);
	BMap.Convertor.transMore(linesPoints,0,callback);
	map.centerAndZoom(linesPoints[linesPoints.length -1],15);
	add_label();
}
</script>

<form id="get00"><input id="c" name="c"  type="hidden"  value=<%=cid %>> </form>
<iframe id="datamap" name="datamap" src="./latest.jsp?cid=<%=cid %>"></iframe>
</body>
